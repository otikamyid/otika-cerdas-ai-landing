...
const CrmFeatures = () => <section className="section v3-crm">
  <div className="v3-shell">
    ...
    <div className="v3-pipeline" data-reveal aria-label="Ilustrasi perjalanan satu prospek konstruksi di sales pipeline PT Megacon Bangun Perkasa">
      <div><span className="v3-small-label">SALES PIPELINE · PT MEGACON BANGUN PERKASA</span><span className="v3-pipeline-caption">Satu prospek, dari tanya sampai siap ditindaklanjuti</span></div>
      <div className="v3-pipeline-columns">{[
        { title: "Prospek masuk", customer: "PT Karya Mandiri", task: "Tanya U Ditch ukuran 50×50", owner: "Ditangkap AI Megacon", color: "sky" },
        { title: "Sedang ngobrol", customer: "PT Karya Mandiri", task: "Butuh 120 unit untuk proyek", owner: "Kebutuhan dikualifikasi", color: "amber" },
        { title: "Siap ditindaklanjuti", customer: "PT Karya Mandiri", task: "Volume & lokasi proyek lengkap", owner: "Siap diteruskan ke sales", color: "green" },
      ].map((item, i) => <div className={`v3-pipeline-column v3-pipeline-${item.color}`} key={item.title}><h3><span />{item.title}<small>0{i + 1}</small></h3><div><strong>{item.customer}</strong><p>{item.task}</p><span>{item.owner}</span></div></div>)}</div>
    </div>
